...
const width = 450;
const height = 195; // increased height for better spacing
...
async function fetchRepoStats(repoName) {
  try {
    const res = await fetch(`https://api.github.com/repos/Naseer-047/${repoName}`, {
      headers: {
    ...
    console.error(`Error fetching stats for ${repoName}:`, err.message);
...
function createCard(title, desc, tags, stars, forks, iconSvg, filename) {
  ...
  tags.forEach(tag => {
    ...
    tagsSvg += `
      <rect x="${currentX}" y="135" width="${tagWidth}" height="20" class="tag" />
      <text x="${currentX + 5}" y="149" class="mono tag-text">${tag}</text>
    `;
    currentX += tagWidth + 10;
  });

  const starTextWidth = stars.toString().length * 8 + 15;
  const forkX = 25 + starTextWidth + 15; // dynamic spacing based on stars text length
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${width} ${height}" width="${width}" height="${height}">
  ...
  <g transform="translate(25, 170)">
    ...
    <!-- Fork -->
    <path d="M48 2c-1.1 0-2 .9-2 2v2.59l3 3V12h-2v2h6v-2h-2V9.59l3-3V4c0-1.1-.9-2-2-2z" fill="none" stroke="var(--text-muted)" stroke-width="1.5" transform="scale(0.8) translate(${forkX - 40}, 0)"/>
    <text x="${forkX + 15}" y="10" font-size="12" class="mono text-muted">${forks}</text>
  </g>
</svg>`;
...
async function main() {
  ...
  const cnsStats = await fetchRepoStats('CNS');
  const gccStats = await fetchRepoStats('GCC-Developer-OS');
  const smartStats = await fetchRepoStats('Smart-Study-Buddy');
  const resumeStats = await fetchRepoStats('AI-Resume-Analyzer');
  ...
  createCard(
    ...
    smartStats.stars, smartStats.forks,
  ...
  createCard(
    ...
    resumeStats.stars, resumeStats.forks,
  ...
  console.log('Project cards generated successfully with real data and perfect spacing!');
}
